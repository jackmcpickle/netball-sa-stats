...
describe('ClubHistory.lastRankedYear', () => {
    it('is the most recent ranked year the club fielded a final-season team', () => {
        const history = ClubHistory.from(
            club,
            [
                row({ year: 2020, isFinal: true }),
                row({ year: 2016, isFinal: true }),
            ],
            [2016, 2020, 2024],
        );
        expect(history.lastRankedYear()).toBe(2020);
    });

    it('ignores non-final rows', () => {
        const history = ClubHistory.from(
            club,
            [row({ year: 2026, isFinal: false })],
            [2024],
        );
        expect(history.lastRankedYear()).toBeNull();
    });

    it('ignores years outside the ranked set', () => {
        const history = ClubHistory.from(
            club,
            [row({ year: 1999, isFinal: true })],
            [2024],
        );
        expect(history.lastRankedYear()).toBeNull();
    });

    it('is null when the club has no results at all', () => {
        const history = ClubHistory.from(club, [], [2024]);
        expect(history.lastRankedYear()).toBeNull();
    });
});
