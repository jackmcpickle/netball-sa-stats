...
(() => {
  ...
  `;
    ...
    function walkIf(el, host) {
        ...
        const hintGet = hintRaw == null ? null : compileAttr(hintRaw);
        const kids = walkChildren(el, host);
        return (vals, ctx, key) => {
            let v = valGet(vals);
    ...
    function walkComponent(el, host) {
        ...
        const styleGet = styleRaw == null ? null : compileAttr(styleRaw);
    ...
    function init() {
        ...
        if (document.readyState === 'loading') {document.addEventListener('DOMContentLoaded', () =>{  api.__dcBoot(); });}
        else
            {api.__dcBoot();}
    }
    hideRawTemplate();
